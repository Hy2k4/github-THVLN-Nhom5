    ...
    if (!empty($cartData)) {
        ...
        try {
            ...
            if (!$stmt_cart->execute()) {
                throw new Exception("Lỗi khi thêm vào bảng 'giohang': " . $stmt_cart->error);
            }
            ...
            foreach ($cartData as $item) {
                ...
                if (!$stmt_order->execute()) {
                    throw new Exception("Lỗi khi thêm vào bảng 'chitietdathang': " . $stmt_order->error);
                }
            }
            ...
            echo "<script>alert('Mua đơn hàng thành công!');</script>";
...
<body>
    <div class="header">
        ...
        <div class="icons">
        <a href="./giohang.php" style="color: white; text-decoration: none;"><button><i class="fas fa-arrow-left"></i> Quay lại</button></a>
        </div>
    </div>

    <div class="container">
        <h2>Thông tin sản phẩm đã chọn</h2>
        <?php
        if (empty($products)) {
    ...
    <div class="order-info">
        ...
        <p>Tên người dùng: <?= htmlspecialchars($username) ?> (chỉ hiện thông tin)</p>
        
        <!-- Kiểm tra nếu có giá trị trong fullname -->
        <p>Họ và tên: <?= !empty($user_info['fullname']) ? htmlspecialchars($user_info['fullname']) : 'Chưa cập nhật' ?></p>
        
        <!-- Kiểm tra nếu có giá trị trong address -->
        <p>Địa chỉ: <?= !empty($user_info['address']) ? htmlspecialchars($user_info['address']) : 'Chưa cập nhật' ?></p>
        
        <!-- Kiểm tra nếu có giá trị trong số điện thoại -->
        <p>Số điện thoại: <?= !empty($user_info['sdt']) ? htmlspecialchars($user_info['sdt']) : 'Chưa cập nhật' ?></p>
        
        <p>Tổng giá trị đơn hàng: <?= number_format($totalPrice, 0, ',', '.') ?> vnđ</p>

        <p>Phương thức thanh toán: <span id="payment-method">Chưa chọn phương thức</span></p>
    </div>
    ...
    <form method="POST" action="" id="payment-form">
        ...
        <button type="submit" name="save_order" <?= empty($products) ? 'disabled' : '' ?> id="btn-submit">mua đơn hàng</button>
    </form>    
